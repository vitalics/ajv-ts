...
import { SchemaBuilder, array, infer, number, object, s, string } from "../src";
...
test("readonly marks schema readOnly", () => {
	const arr = s.array(s.string()).readonly();
	expect(arr.schema).toMatchObject({
		type: "array",
		items: { type: "string" },
		readOnly: true,
	});
	expect(arr.parse(["a"])).toEqual(["a"]);
});
