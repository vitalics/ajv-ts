...
test("preprocess should transform input result", () => {
  const envParsingSchema = s
    .boolean()
    .preprocess((x) => String(x) === "true" || String(x) === "1");
...
test("refine should throws custom error", () => {
  ...
  expect(result.error?.message).toBe(
    'Array should contains only 1 "active" element',
  );
});
...
test("refine should throws default error", () => {
  const Schema = s
    ...
    .refine((arr) => {
      const subArr = arr.filter((el) => el.active === true);
      if (subArr.length > 1)
        return new Error('Array should contains only 1 "active" element');
    });
