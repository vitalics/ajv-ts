...
test("exclusive minimum adds exclusiveMinimum", () => {
  ...
  assertType<{ readonly type: "number"; readonly exclusiveMinimum: 5 }>(num._schema);
});
...
test("gt/gte/lt/lte produce correct schema keys", () => {
  ...
  assertType<{ readonly type: "number"; readonly exclusiveMinimum: 0 }>(gt._schema);

  const gte = s.number().gte(0);
  assertType<{ readonly type: "number"; readonly minimum: 0 }>(gte._schema);

  const lt = s.number().lt(0);
  assertType<{ readonly type: "number"; readonly exclusiveMaximum: 0 }>(lt._schema);
...
test("positive/nonnegative/negative/nonpositive use range keys", () => {
  assertType<{ readonly type: "number"; readonly exclusiveMinimum: 0 }>(s.number().positive()._schema);
  assertType<{ readonly type: "number"; readonly minimum: 0 }>(s.number().nonnegative()._schema);
  assertType<{ readonly type: "number"; readonly exclusiveMaximum: 0 }>(s.number().negative()._schema);
  assertType<{ readonly type: "number"; readonly maximum: 0 }>(s.number().nonpositive()._schema);
});
...
test("multipleOf and step add the key", () => {
  ...
  assertType<{ readonly type: "number"; readonly multipleOf: 0.1 }>(step._schema);
});

test("format adds the key", () => {
  const num = s.number().format("int32");
  assertType<{ readonly type: "number"; readonly format: "int32" }>(num._schema);
});
